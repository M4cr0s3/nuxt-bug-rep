...
const handleSubmit = async () => {
	if (isAgree.value) {
		try {
			const response = await $fetch<{ success: boolean, message: string }>('/auth/sign_up', {
			...
			<div class='form'>
				...
				<form @submit.prevent='handleSubmit' class='flex flex-col'>
					...
					<div class='flex items-start mt-6'>
						<input id='default-checkbox' v-model='isAgree' type='checkbox' value='' class='checkbox w-6 h-6'>
				...
				<div class='mt-6 font-Ml text-2xl font-bold text-main text-center'>
					{{ error }}
				</div>
